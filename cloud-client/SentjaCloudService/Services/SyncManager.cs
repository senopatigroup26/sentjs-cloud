...
public class SyncManager : IDisposable
{
    ...
    public async Task InitialSyncAsync(CancellationToken ct = default)
    {
        ...
        {
            var result = await _apiClient.GetFilesAsync(new FileListRequest { Page = page, PageSize = 50 });
    ...
    public async Task UploadFileAsync(string fullPath)
    {
        try
        {
            ...
            if (info.Length == 0) return; // skip placeholder

            var relativePath = Path.GetRelativePath(_syncRoot, fullPath).Replace('\\', '/');
            _logger.LogInformation("Syncing: {Path} ({Size} bytes)", relativePath, info.Length);
            ...
            _logger.LogInformation("Uploaded to Hetzner: {Remote}", remotePath);
            ...
            if (!res.Success)
            {
                _logger.LogWarning("Backend record failed: {Error}", res.Error);
                return;
            }

            // 4. Dehydrate: replace local file with empty placeholder
            Dehydrate(fullPath);
            _logger.LogInformation("Dehydrated: {Path}", relativePath);
        }
        catch (Exception ex)
        {
            _logger.LogError("Upload failed for {Path}: {Error}", fullPath, ex.Message);
        }
    }
